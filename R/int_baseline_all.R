...
int_baseline_all <- function(locationBaseline, locationReflectance, locationTransmittance, Averaging=1, writeLoc = NULL, writePrefix = "") {
  ...
  adjustment_matrix <- add_column(adjustment_matrix,TransTrans = adjvec)
  adj2 <- adjvec * baseline_data$Reflectance/100
  adjustment_matrix <- add_column(adjustment_matrix,TTR = adj2)

  #Apply the baseline adjustment
  suppressWarnings(plant_data_adj <- merge(transmittance_data,reflectance_data,by="Wavelength"))
  plant_data_adj <- add_column(plant_data_adj,TTR=adjustment_matrix$TTR)
  plant_data_adj <- add_column(plant_data_adj,RefAdj=plant_data_adj$Reflectance-(plant_data_adj$TTR)*100)
  plant_data_adj <- add_column(plant_data_adj,Absorbance=100-plant_data_adj$RefAdj-plant_data_adj$Transmittance)

  #generate a graph
  adj_data_plot <- int_graph(select(plant_data_adj,"Wavelength","Transmittance","RefAdj","Absorbance"))

  #optionally write data
  if(!is.null(writeLoc)){
    setwd(writeLoc)
    write_csv(baseline_data,paste0(writePrefix,"Baseline reflectance.csv"))
    write_csv(reflectance_data,paste0(writePrefix,"Plant reflectance.csv"))
    write_csv(transmittance_data,paste0(writePrefix,"Plant transmittance.csv"))
    write_csv(plant_data_adj, paste0(writePrefix,"Compiled plantdata.csv"))
  }
